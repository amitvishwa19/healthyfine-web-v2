...
export default function AppointmentCrud() {
    ...
    useEffect(() => {
        if (mData.type === 'edit') {
            if (mData?.appointment) {
                form.reset({
                    id: mData?.appointment?.id || "",
                    doctorId: mData.appointment.doctorDetails?.id || "",
                    patientId: mData.appointment.patientDetails?.id || "",
                    serverId: orgId || "",
                    date: mData.appointment.date || "",
                    slot: mData.appointment.slot || null,
                    time: mData.appointment.time || "",
                    type: mData.appointment.type || { type: 'clinic', status: false, charge: 250, icon: 'hospital' },
                    note: mData.appointment.note || "",
                    status: mData.appointment.status || "pending"
                });
            }
        } else {
        }
    ...
    function parseTime(t) {
        ...
        return compareTime < now && (moment().format('DD') === moment(mData?.date).format('DD'));
    }
    ...
    const { execute } = useAction(updateAppointment, {
        onSuccess: (data) => {
            ...
            refreshServer().then(
                toast.success('New appointment created successfully', { id: 'update-appointment' })
            )
        },
        onError: (error) => {
            setLoading(false)
    ...
    return (
        <Dialog open={isModalOpen} onOpenChange={() => { handleOpenChange() }}>
            ...
            <DialogContent open={true} className="p-0 min-w-[90%]  dark:bg-darkPrimaryBackground [&>button:last-child]:hidden overflow-hidden">
                            ...
                            <div className='flex flex-col gap-6'>
                                                    ...
                                                    {(doctor?.setting?.timing ?? hospitalDefaultSettings?.timing)?.map((item, index) => {
                                                        ...
                                                        return (
                                                            ...
                                                            <Button
                                                                ...
                                                                className={`flex flex-col p-4 h-12 
                                                                         bg-primary/20 items-center justify-center rounded-md 
                                                                        dark:bg-darkFocusColor/60  flex-1 mx-1
                                ...
                                <div className='flex flex-row items-center gap-2 justify-end'>
                                    ...
                                    <Button disabled={loading} type="submit" variant='save' size='sm' className="">
                                        ...
                                        {modalType?.type === 'add' ? 'Book Now' : 'Save'}
                                    </Button>
                                </div>
                            </div>
                        </form>
